...
`serve` and `install` accept `--port` (default 8200) and `--db` (default `~/.config/oauth-husk/oauth.db`). The database and signing key are created automatically on first run.
